...
public class PermissionChecker {
        ...
        if (debugMode) {
            ...
            for (String permission : requestPermissions) {
                if (!allPermissions.contains(permission)) {
                    // 请不要申请危险权限和特殊权限之外的权限
                    throw new IllegalArgumentException("The " + permission + " is not a dangerous permission or special permission, please do not apply dynamically");
                }
            }
        }
        return true;
     ...
     * @param requestPermissions 请求的权限组
     */
    public static void checkStoragePermission(Context context, List<String> requestPermissions) {
        // 如果请求的权限中没有包含外部存储相关的权限，那么就直接返回
        if (!requestPermissions.contains(Permission.MANAGE_EXTERNAL_STORAGE) && !requestPermissions.contains(Permission.READ_EXTERNAL_STORAGE) && !requestPermissions.contains(Permission.WRITE_EXTERNAL_STORAGE)) {
            return;
        }

        // 是否适配了分区存储
        boolean scopedStorage = PermissionKit.isScopedStorage(context);

        XmlResourceParser parser = PermissionKit.parseAndroidManifest(context);
        if (parser == null) {
            return;
        }
        ...
        try {
            ...
            do {
                ...
                boolean requestLegacyExternalStorage = parser.getAttributeBooleanValue(PermissionKit.ANDROID_NAMESPACE, "requestLegacyExternalStorage", false);
     ...
     * @param requestPermissions 请求的权限组
     */
    public static void checkLocationPermission(Context context, List<String> requestPermissions) {
        if (context.getApplicationInfo().targetSdkVersion >= Build.VERSION_CODES.S) {
            if (requestPermissions.contains(Permission.ACCESS_FINE_LOCATION) && !requestPermissions.contains(Permission.ACCESS_COARSE_LOCATION)) {
        ...
        for (String permission : requestPermissions) {
            if (Permission.ACCESS_FINE_LOCATION.equals(permission) || Permission.ACCESS_COARSE_LOCATION.equals(permission) || Permission.ACCESS_BACKGROUND_LOCATION.equals(permission)) {
                continue;
            }
    ...
    public static void checkTargetSdkVersion(Context context, List<String> requestPermissions) {
        ...
        if (requestPermissions.contains(Permission.BLUETOOTH_SCAN) || requestPermissions.contains(Permission.BLUETOOTH_CONNECT) || requestPermissions.contains(Permission.BLUETOOTH_ADVERTISE)) {
        ...
        } else if (requestPermissions.contains(Permission.ACCESS_BACKGROUND_LOCATION) || requestPermissions.contains(Permission.ACTIVITY_RECOGNITION) || requestPermissions.contains(Permission.ACCESS_MEDIA_LOCATION)) {
            targetSdkMinVersion = Build.VERSION_CODES.Q;
        } else if (requestPermissions.contains(Permission.REQUEST_INSTALL_PACKAGES) || requestPermissions.contains(Permission.ANSWER_PHONE_CALLS) || requestPermissions.contains(Permission.READ_PHONE_NUMBERS)) {
     ...
     * @param requestPermissions 请求的权限组
     */
    public static void checkManifestPermissions(Context context, List<String> requestPermissions) {
        HashMap<String, Integer> manifestPermissions = PermissionKit.getManifestPermissions(context);
     ...
     */
    public static void checkManifestPermission(HashMap<String, Integer> manifestPermissions, String checkPermission, int maxSdkVersion) {
        ...
        if (manifestMaxSdkVersion < maxSdkVersion) {
            ...
            throw new IllegalArgumentException("The AndroidManifest.xml file " +
                    ...
                    (maxSdkVersion != Integer.MAX_VALUE ? "the minimum requirement for maxSdkVersion is " + maxSdkVersion : "please delete the android:maxSdkVersion=\"" + manifestMaxSdkVersion + "\" attribute"));
        }
    }
     ...
     * @param requestPermissions 请求的权限组
     */
    public static void optimizeDeprecatedPermission(List<String> requestPermissions) {
        if (!PermissionKit.isAndroid12() && requestPermissions.contains(Permission.BLUETOOTH_SCAN) && !requestPermissions.contains(Permission.ACCESS_COARSE_LOCATION)) {
            // 自动添加定位权限，因为在低版本下获取蓝牙扫描的结果需要此权限
            requestPermissions.add(Permission.ACCESS_COARSE_LOCATION);
        }

        // 如果本次申请包含了 Android 12 蓝牙扫描权限
        if (!PermissionKit.isAndroid12() && requestPermissions.contains(Permission.BLUETOOTH_SCAN)) {
