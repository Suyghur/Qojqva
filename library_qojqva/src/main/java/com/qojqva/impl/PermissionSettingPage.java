...
public final class PermissionSettingPage {
     ...
     */
    public static Intent getSmartPermissionIntent(Context context, List<String> permissions) {
        // 如果失败的权限里面不包含特殊权限
        if (permissions == null || permissions.isEmpty() || !PermissionKit.containsSpecialPermission(permissions)) {
            return getApplicationDetailsIntent(context);
        }

        if (PermissionKit.isAndroid11() && permissions.size() == 3 &&
